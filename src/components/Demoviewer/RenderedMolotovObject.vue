<template>
  <div class="rendered-molotov-object" :style="{left: `${position.x}px`, top: `${position.y}px`}">
    <div class="projectile" v-show="tick >= startTime && tick <= endTime">9</div>
    <div
      class="firecloud"
      v-show="tick >= endTime && tick <= goneTime"
      :style="{background: `rgba(255, 69, 0, ${alpha})`}"
    >
...
export default {
  ...
  computed: {
    endTime() {
      return this.molotov.Trajectory[this.molotov.Trajectory.length - 1].Time;
    },
    startTime() {
      return this.molotov.Trajectory[0].Time;
    },
    goneTime() {
  ...
  methods: {
    UpdatePosition() {
      for (
        ...
        idx < this.molotov.Trajectory.length;
        idx++
      ) {
        let position = this.molotov.Trajectory[idx];
